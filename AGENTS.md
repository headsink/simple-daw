...
**Bug-fix pass #3 — Complete (code changes).** All 13 fixes applied across 11 files; build green; 12/12 MidiClip tests pass. **Audio regression investigation — IN PROGRESS.** User reports: after rebuilding, no sound from audio tracks, sequencer, or synth — but the **master peak meter IS moving**. The audio rendering path in `MainComponent::getNextAudioBlock` is unchanged, and the peak meter is the last measurement before the buffer is handed to the audio device. The problem is between the device's output buffer and the speakers. Top suspects: (a) `MidiOutputRouter` `CriticalSection` lock added on the audio thread path (kernel-mode `EnterCriticalSection` on contention can stall the audio thread past the buffer size → device underrun), (b) `Recorder::audioDeviceAboutToStart` 300 s pre-allocation (~110 MB on a memory-constrained system can delay/partial-fail the device start). User confirms build is correct, status bar shows device, peak meter moves, restart does not help. NOT a Windows audio routing issue per user.
...
**Bug-fix pass #3 — IN PROGRESS.** Three `DocumentWindow` double-frees being fixed first (atomic `closing` flag in `closeButtonPressed`, plus destructors call `closeButtonPressed()` instead of synchronous `delete`). See full bug list in the "Known issues" section below.
...
- ~~**`Recorder::audioDeviceIOCallbackWithContext` allocated on audio thread.**~~ **Fixed**: pre-allocate 300 s in `audioDeviceAboutToStart`; `audioDeviceIOCallbackWithContext` only grows if capacity is exceeded (no shrink, no fresh alloc on first recording).
