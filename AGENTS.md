...
- `src/tracks/AudioTrack.{h,cpp}` — `class AudioTrack` owning `AudioTrackSource` + scratch buffer + `gain`/`pan`/`mute`/`solo` atomics + `std::unique_ptr<juce::AudioPluginInstance> plugin` + `std::unique_ptr<juce::PluginDescription> pluginDesc` + `std::atomic<bool> pluginBypass` + `juce::MidiBuffer pluginMidiBuffer` + `renderInto()`. `setPlugin(plugin, desc)` calls `prepareToPlay` + `setBusesLayout` (stereo in/out) and stores the `PluginDescription` so it can be serialised in `.sdaw`. `renderInto` runs `plugin->processBlock(scratchBuffer, pluginMidiBuffer)` after the source renders, then applies `gain` to the scratch buffer, measures peak post-gain via `getMagnitude` (stored in `std::atomic<float> peak` via max-hold), then adds to the destination with `panGain` (skipped if muted, skipped on solo-exclusion). `getPeakRef()` exposes the atomic to the meter UI. `getPluginDesc()` returns the stored description. `loadFileAsync` is lifetime-safe via a `std::shared_ptr<std::atomic<int>>` token that gets bumped in the destructor; the async callback checks the token before touching `this`.
...
- **`PluginChooserDialog::closeButtonPressed` defers `delete this`** via `MessageManager::callAsync` to avoid fragile synchronous destruction after `exitModalState`.
...
**Piano roll copy/paste + undo/redo — Complete.** Selection (click / shift-click / drag-rect), clipboard (Ctrl+C/V/X), undo/redo (Ctrl+Z/Y) with snapshot-based stacks, toolbar buttons. Selection cleared after undo/redo/paste to avoid stale indices.
**Bug-fix pass (items 1-6) — Complete.** Stuck-note in piano roll keys, plugin scan off message thread, async audio file load, loadSession BPM default, recorder sample-rate defensive default.

**Recommended next: pick a stretch task.** Core DAW is functional: multi-track audio + VST3 inserts + A/B loop + sequencer + piano roll (selection, clipboard, undo/redo, velocity lane, zoom, scroll) + master meter + ASIO settings + JSON save/load + recording + MIDI output routing. Remaining items are polish.

### Stretch tasks (pick any)
- **Release build** benchmark.
- **App icon** via `juce_add_app_icon` or manual `.ico`.
- **Track reorder via drag** — `juce::DragAndDropContainer` on `tracksContainer`.
- **Drag-and-drop audio files** onto a track row — `juce::FileDragAndDropTarget` (one override).
- **Spacebar = play/stop sequencer** — global `KeyListener` on `MainComponent`.
...
- **Piano roll selection** — click to select, shift-click to add, drag empty space to marquee-select multiple notes. Selected notes render in amber, velocity bars in amber.
- **Piano roll copy/paste/cut** — `Ctrl+C` copies selected (normalised to start at beat 0, lowest pitch as 0); `Ctrl+V` pastes at `pasteBeatHint` (beat 0 for v1); `Ctrl+X` cuts (copy + delete). Process-local clipboard.
- **Piano roll undo/redo** — `Ctrl+Z` / `Ctrl+Y` (or `Ctrl+Shift+Z`). Snapshot-based stacks in `MidiClip` (cap 200 entries). Every mutation pushes a state: drag (move/resize/velocity), create, delete (right-click or `Del`), paste, cut, Clear All. State is pushed at the natural point of each operation (mouseDown for single-shot, first mouseDrag for drags, mouseUp for create).
- **Toolbar buttons** — `Undo` / `Redo` / `Copy` / `Paste` (next to existing zoom and clear). `Ctrl+A` selects all.
- **Arrow-key nudge** for selected notes (up/down = pitch ±1, left/right = beat ±snap).
- **Plugin scan on `PluginScanThread`** — enumerates `*.vst3` files in `C:\Program Files\Common Files\VST3` and `%APPDATA%\VST3`, calls `AudioPluginFormat::findAllTypesForFile` per file (which doesn't touch the `KnownPluginList`), and posts each new plugin to the message thread for `addType`. A `Timer` in the chooser dialog polls the count and shows `"Scanning... (N found)"`.
- **Async audio file load** — `AudioTrackSource::loadFileAsync` reads on a `std::thread` and posts the buffer back via `callAsync`. Lifetime safety via `std::shared_ptr<std::atomic<int>>` tokens bumped in the destructors of `AudioTrackSource` and `AudioTrack`. Used by `TrackRow::openFile`, `MainComponent::loadSession`, and `MainComponent::toggleRecording`. `TrackRow` shows `(loading...) filename.ext` while in flight and disables Play/Stop.
- **BPM restore** — `loadSession` now uses `json["bpm"].isVoid()` to default to 120 for old `.sdaw` files.
- **Recorder sample rate guard** — `stopRecording` falls back to 48000 Hz if `recordSampleRate` is 0.
- **Piano roll window size** — bumped to 820×560 to fit the wider toolbar (Undo/Redo/Copy/Paste). Focus grabbed automatically on open.
- **VST3 plugin state in `.sdaw`.** `AudioTrack` stores the original `PluginDescription` (set in `setPlugin`, also captured in the `TrackRow` open-chooser callback). `MainComponent::saveSession` writes a `plugin` sub-object per track: `identifier` (`PluginDescription::createIdentifierString()`), `name`, `manufacturer`, `format`, `bypass`, and `state` (the result of `AudioProcessor::getStateInformation` base64-encoded via `juce::Base64::toBase64`). `MainComponent::loadSession` reads the sub-object, matches the identifier against `pluginHost.getKnownList().getTypes()`, async-creates the instance, decodes the state via `juce::Base64::convertFromBase64` into a `MemoryOutputStream`, calls `setStateInformation`, and restores the bypass flag. Lifetime safety preserved via the same `std::shared_ptr<std::atomic<int>>` token as the audio file load. Tracks whose plugin is no longer in the known list load as plugin-less; the audio file + gain/pan/mute/solo/loop still restore.
- **Per-track peak meter** — `src/ui/TrackMeterComponent.h` is a smaller (50 px wide, ~10 px tall) log-scale bar copied from `PeakMeterComponent`. `AudioTrack` owns `std::atomic<float> peak{0.0f}`; `renderInto` applies `gain` to the scratch buffer, then `getMagnitude`-measures the post-gain signal and stores via max-hold (`if (blockPeak > peak.load()) peak.store(blockPeak)`), then adds to the destination with `panGain`. The meter uses the same `exchange(0.0f)` read+reset + 0.88 decay pattern as the master meter. Sits in `TrackRow` between the time label and the gain slider; shows the level of the source-after-fader signal (drops to 0 on mute / solo-exclusion).
