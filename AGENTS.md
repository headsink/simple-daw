...
**Bug-fix pass #2 (3 sync `delete this` heap-corruption bugs + MIDI-out `sendAllNotesOff` + per-track gain smoothing + `~AudioTrack` releaseResources) — Complete.**
**Refactor (createTrackFromFileAsync helper) — Complete.** ~50 lines of duplication between `loadSession` and `toggleRecording` removed.
**Stretch: spacebar = sequencer play/stop — Complete.** `MainComponent::keyPressed` toggles `midiTrack.play()/stop()`.
**Stretch: drag-and-drop audio files onto a track row — Complete.** `TrackRow` implements `juce::FileDragAndDropTarget`; accepts `.wav`/`.aif`/`.aiff`/`.flac`/`.mp3`/`.ogg`. Row highlights blue on drag-over.

**Recommended next: pick a stretch task.** Core DAW is functional: multi-track audio + VST3 inserts + A/B loop + sequencer + piano roll (selection, clipboard, undo/redo, velocity lane, zoom, scroll) + master meter + ASIO settings + JSON save/load + recording + MIDI output routing + per-track gain smoothing + drag-and-drop + spacebar transport. Remaining items are polish.
...
- **Per-track gain smoothing** — `AudioTrack` owns a `juce::SmoothedValue<float> gainSmoothed` (50 ms ramp, reset in `prepareToPlay`). `setGain` updates the target; `renderInto` uses `applyGainRamp` (per-channel, start/end values) for sample-accurate transitions. Eliminates zipper noise on slider drags.
- **`AudioTrack::releaseResources()` now called from `~AudioTrack`** so plugin + scratch buffer resources are freed even when tracks are destroyed via `loadSession` or `removeTrack` (not just the standard `MainComponent::releaseResources` path).
- **`createTrackFromFileAsync(juce::File, float initialGain)` helper** in `MainComponent` — extracted the ~50 lines of duplicated "create track + row + push + async load + on-complete update" between `loadSession` and `toggleRecording`. Returns the new `TrackRow*` so the caller can configure further (e.g. `loadSession` sets pan/mute/solo/loop + the saved plugin state).
- **Three `DocumentWindow::closeButtonPressed` deferred deletes** — `SettingsWindow`, `PluginEditorWindow`, and `PianoRollWindow` now defer `delete this` via `MessageManager::callAsync` (same pattern as `PluginChooserDialog`). Closes on those windows no longer risk the `_CrtIsValidHeapPointer` heap-corruption bug.
- **`MainComponent` alive flag** — `std::shared_ptr<bool> alive` is set to `false` in `~MainComponent`; the `onClosed` callbacks for `pianoRollWindow` and `settingsWindow` capture it and no-op if the parent is gone. Protects against the race where a subwindow is closed and the app is shut down within one message-loop tick.
- **`loadSession` MIDI-out cleanup** — calls `sendAllNotesOff()` on the previous output before opening the saved one, matching the combo's `onChange` behavior. Closes the output if the saved value is `(none)`.
- **Spacebar = sequencer play/stop** — `MainComponent` overrides `keyPressed` and toggles `midiTrack.play()/stop()`. `setWantsKeyboardFocus(true)` + `grabKeyboardFocus()` in the constructor. Slider text boxes are non-editable, so the spacebar isn't swallowed.
- **Drag-and-drop audio files onto a track row** — `TrackRow` now implements `juce::FileDragAndDropTarget`. Accepts `.wav`/`.aif`/`.aiff`/`.flac`/`.mp3`/`.ogg`. On drag-enter the row highlights blue; on drop the existing `loadFileAsync` is used (with the alive flag guard) and the name label refreshes on completion. First valid audio file in a multi-file drop is used.
...
**All core + selected stretch features complete. Remaining items:** Release build, app icon, track reorder via drag.
